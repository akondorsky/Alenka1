...
begin
    E_Zayv.Text:=Dmod.Qry_RegTi.FieldByName('ZAYV').AsString;
    E_Address.Text:=Dmod.Qry_RegTi.FieldByName('ADRESS').AsString;
    E_INN.Text:=Dmod.Qry_RegTi.FieldByName('INN').AsString;
    E_KPP.Text:=Dmod.Qry_RegTi.FieldByName('KPP').AsString;
    E_Ogrn.Text:=Dmod.Qry_RegTi.FieldByName('OGRN').AsString;
    E_Bank.Text:=Dmod.Qry_RegTi.FieldByName('BANK').AsString;
    E_BIK.Text:=Dmod.Qry_RegTi.FieldByName('BIK').AsString;
    E_CountR.Text:=Dmod.Qry_RegTi.FieldByName('SCHET_R').AsString;
    E_CountKor.Text:=Dmod.Qry_RegTi.FieldByName('KORSCH_R').AsString;
    if Dmod.Qry_RegTi.FieldByName('TYPE').AsInteger = 0 then
        Rg_Type.ItemIndex:=0
      else
        Rg_Type.ItemIndex:=1;
end;

procedure TRegTiEdit_F.FindStdPrice;
begin
end;
...
try
 try
  id:=Dmod.Qry_RegTi.FieldByName('ID').AsInteger;
  s:='update reg_ti set ZAYV=:p0,ADRESS=:p1,INN=:p2,KPP=:p3,OGRN=:p4,BANK=:p5,SCHET_R=:p6,KORSCH_R=:p7,BIK=:p8,TYPE=:p9 ' ;
  s:=s+' where ID=:p10 ';
  DMOd.Qry_Add.Close;
...
function TRegTiEdit_F.ZayvExist(FStr: String): Boolean;
begin
